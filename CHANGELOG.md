...
- **An ASGI application's lifespan shutdown was skipped when background
  ...
  application down. Every task now gets 3 s in all, is then cancelled, and a
  task that swallows its cancellation is named in the log and left behind;
  lifespan shutdown runs after. Work that finishes inside the 3 s finishes.
