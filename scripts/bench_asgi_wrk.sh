...
m0()      { bin/m0serve bareapp.asgi:application --app-dir apps/asgi_bare --port 8080 > /dev/null 2>&1 & pid=$!; }
...
python3 "$(dirname "$0")/bench_record.py" asgi_wrk_hello "$OUT" \
  --meta "client=wrk -t2 -c$CONNS -d$DUR, browser headers" --meta "app=apps/asgi_bare" --meta "rounds=$ROUNDS" \
