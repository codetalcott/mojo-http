...
- **Not the fastest server on raw throughput**, and
  ...
  Granian per measured core on bare WSGI, 1.06x uvicorn on ASGI at 16
  connections (0.74x against uvicorn with uvloop) and 1.22x at 256.
...
| `m0-http` | Router, content negotiation, ETag, response cache, SSE, WebSockets, auth, CORS, config, health, logging, multi-worker supervisor, cross-worker broadcast bus, HTTP client, request-parsing hardening | 580 |
...
- **Concurrency is loops, and optionally a handler pool behind each.**
  ...
  *both* modes while p50 does not move at all. **`--blocking-threads N` is
  ...
  located**: normalized per measured core, 85.2k against 100.0k rps/core on
  a bare callable — about 0.85x. The split says where it goes. `apps/hello`,
  the same server with no Python in the path, runs at 115.9k rps/core —
  above Granian's end-to-end rate — so m0serve's own bridge costs 1.36x, and
...
uv run poe test-all         # 1005 unit tests, then compiles every example
