...
| `m0-http` | Router, content negotiation, ETag, response cache, SSE, WebSockets, auth, CORS, config, health, logging, multi-worker supervisor, cross-worker broadcast bus, accept sharing, HTTP client, request-parsing hardening, view table, fragment-or-page | 659 |
...
uv run poe test-all         # 1108 unit tests, then compiles every example
