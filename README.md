...
- **Where it stands on throughput**, from [docs/BENCHMARKS.md](docs/BENCHMARKS.md):
  <!-- num:asgi-vs-uvloop@2 -->1.62<!-- /num -->x uvicorn with uvloop on bare ASGI at 16 connections
  (<!-- num:asgi-vs-uvicorn@2 -->2.31<!-- /num -->x `uvicorn --loop asyncio`), on <!-- num:asgi-m0-cores@1 -->1.6<!-- /num -->
  measured cores where uvicorn has one; the fast-request tail under mixed
  load ahead of uvicorn in every recorded run; and <!-- num:m0-vs-granian-rps@2 -->1.01<!-- /num -->x
  Granian on bare WSGI at one worker and one handler thread each
  (<!-- num:m0-per-granian@2 -->1.02<!-- /num -->x per measured core). Every figure is rendered from a
...
| `m0-http` | Router, content negotiation, ETag, response cache, SSE, WebSockets, auth, CORS, config, health, logging, multi-worker supervisor, cross-worker broadcast bus, accept sharing, the Mojo host, HTTP client, request-parsing hardening, view table, HTML builder and fragment, fragment-or-page, url_for and Query, form bodies, signed session cookies and CSRF | 878 |
...
- **Concurrency is loops, and optionally a handler pool behind each.**
  ...
  *both* modes while p50 does not move at all. **`--blocking-threads N` is
  ...
  located**: one worker and one handler thread each, <!-- num:m0-w1-rps-k@1 -->199.3<!-- /num -->k
  against <!-- num:granian-w1-rps-k@1 -->197.4<!-- /num -->k rps on a bare callable, <!-- num:m0-wsgi-rps-k@1 -->112.0<!-- /num -->k
  against <!-- num:granian-rps-k@1 -->109.7<!-- /num -->k per measured core — about <!-- num:m0-per-granian@2 -->1.02<!-- /num -->x. The
  split prices each layer. `apps/hello`, the same server with no Python
  in the path, runs at <!-- num:hello-rps-k@1 -->209.5<!-- /num -->k rps/core; the bare app run
  inline on that loop, one thread, runs at
  <!-- num:m0-loop-rps-k@1 -->126.1<!-- /num -->k, so m0serve's own bridge costs <!-- num:bridge-tax@2 -->1.66<!-- /num -->x.
...
uv run poe test-all         # 1420 unit tests, then compiles every example
