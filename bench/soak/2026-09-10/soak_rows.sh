#!/bin/bash
# The 2026-09-10 re-soak's six rows against this tree's bin/m0serve (1.0.0),
# one after another, driver output per row. Same driver, same populations,
# same durations and churn cadences as 2026-09-07; the baselines were
# re-captured today against the rebuilt subjects.
#
# textshelf's DATABASE_URL carries NO password: libpq reads ~/.pgpass for
# the `postgres` role, so the scratch database is reachable without a
# secret in this file (2026-09-07's copy had one redacted by hand).
cd "$(dirname "$0")/../../.."
OUT=./out
BIN=$PWD/bin/m0serve
mkdir -p "$OUT"
...
export DATABASE_URL="postgres://postgres@localhost:5432/textshelf_soak" DJANGO_SETTINGS_MODULE=config.settings.local
