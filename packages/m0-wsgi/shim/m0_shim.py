...
def _detect(app):
    ...
    if not callable(target):
        raise ValueError(
            ...
            'async app(scope, receive, send), got %s' % type(target).__name__
        )
...
def _lifespan_startup():
        ...
        if not started.done():
            if t == 'lifespan.startup.complete':
                ...
                started.set_exception(
                    RuntimeError(
                        'ASGI lifespan startup failed: %s' % message.get('message', '')
                    )
                )
    ...
    async def wait_started():
        await asyncio.wait(
            {_lifespan_task, started}, return_when=asyncio.FIRST_COMPLETED
        )
...
def lifespan_shutdown():
    ...
    if _loop is None or _loop.is_closed():
        return
    if _lifespan_ok and _lifespan_task is not None and not _lifespan_task.done():
        _lifespan_rq.put_nowait({'type': 'lifespan.shutdown'})
        try:
            _loop.run_until_complete(
                asyncio.wait_for(asyncio.shield(_lifespan_task), 5.0)
            )
        except Exception:
            pass
    if _lifespan_task is not None and not _lifespan_task.done():
        ...
        try:
            _loop.run_until_complete(
                asyncio.gather(_lifespan_task, return_exceptions=True)
            )
        except Exception:
            pass
    _lifespan_task = None
...
def _scope_from_environ(environ):
    ...
    raw_path = (environ.get('SCRIPT_NAME', '') + environ.get('PATH_INFO', '')).encode(
        'latin-1'
    )
    return {
        'type': 'http',
        ...
        'headers': headers,
        'client': (
            (environ['REMOTE_ADDR'], int(environ.get('REMOTE_PORT') or 0))
...
async def _serve_one_buffered(scope, body):
            ...
            if message.get('more_body', False):
                if stream_deadline[0] is None:
                    ...
                    raise RuntimeError(
                        ...
                        'docs/WSGI_VS_ASGI.md section 8' % _ASGI_STREAM_GRACE
                    )
    ...
    if captured['status'] is None:
        raise RuntimeError(
            'ASGI application completed without sending ' 'http.response.start'
        )
    status = captured['status']
    headers = [
        (n.decode('latin-1'), v.decode('latin-1')) for n, v in captured['headers']
    ]
    return ('%d %s' % (status, _reasons.get(status, '')), headers, b''.join(chunks))
...
class _M0Broadcast:
    ...
    def __init__(self):
        ...
        self._fds = [
            int(x)
            for x in os.environ.get('M0_BUS_WRITE_FDS', '').split(',')
            if x.strip()
        ]
        self._next_id = None
...
def _exec_on_ws_message(slot, opcode, payload):
    ...
    if opcode == 1:
        ...
        inbox.put_nowait(({'type': 'websocket.receive', 'bytes': payload}, cost))
    ...
    def _on_submit():
        while True:
            try:
                ...
                return
            if len(data) == 8:
                _exec_put(('job', int.from_bytes(data, 'little', signed=True)))
            elif (
                len(data) >= 9
                and data[0] == _TAG_JOB_BATCH
                and (len(data) - 1) % 8 == 0
            ):
...
def _ws_subprotocols(headers):
    for name, value in headers:
        if name == b'sec-websocket-protocol':
            return [p.strip() for p in value.decode('latin-1').split(',') if p.strip()]
    return []
