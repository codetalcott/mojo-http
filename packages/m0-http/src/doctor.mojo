"""`Report`: a `--doctor`'s facts and checks, rendered as one JSON object.

Both hosts have a doctor -- `m0serve --doctor` and a Mojo host app's own
(`m0_host`) -- and one contract: print one JSON object, start nothing, and
exit with **the code the server itself would exit with for the same
arguments**. This is the pure half they share, so an agent parses one
shape: `ok`, `exit`, grouped facts, and `checks`, each failed check
carrying the `fix` and the `exit` it would cause. It lived in m0-wsgi
until 2026-09-19; `m0_wsgi.doctor` re-exports it, and says what m0serve's
own contract adds.

`product` is the report's first key, whose value is the version:
`"m0serve":"1.5.0"`, `"m0_host":"1.5.0"`.

Structure-of-arrays throughout: these are Strings, and the parallel lists
keep insertion order, which is what makes the output diffable across runs.
"""
...
struct Report(Copyable, Movable):
    ...
    var version: String
    """The product's version, passed in so this module needs no import from
    a CLI and stays trivially constructible in tests."""
    var product: String
    """The report's first key: `m0serve`, or `m0_host`."""

    def __init__(out self, var version: String, var product: String = "m0serve"):
    ...
    def render(self) -> String:
        ...
        var out = String("{")
        out += escape_json_string(self.product) + ":" + escape_json_string(self.version)
