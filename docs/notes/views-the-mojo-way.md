...
Two apps are the worked examples, and both were converted under a green
smoke rather than written fresh, so the wire output is known not to have
moved. `apps/notes_api` keeps its handler struct — it uses `after_response`
for CORS and owns a static mount and a health registry, hooks the table
does not cover — and calls `Views.dispatch` from a `func` with no chain in
it. `apps/fragment_notes` needs no hook at all, so `ViewService` is its
handler and `main` is the table, the store, and serve. A first draft of
this note came with a third notes app, `apps/views_pattern`, built to show
the shape; it was dropped rather than landed, because the tree does not
need three notes apps and the shape is shown by the two that exist.
...
`Views` is gated by `test_views.mojo` and the read/write split by `poe
sabotage-views`. The two converted apps are gated on the wire by
`smoke-notes` and `smoke-fragment-notes`, which passed unchanged across
the conversion — that, and not a diff of the sources, is what "the wire
output did not move" rests on.
