...
- **Cores are measured, not configured.** Each table's `cores` column is
  ...
  `--workers 1` uses ~<!-- num:granian-w1-cores@2 -->1.80<!-- /num --> cores
...
| question | answer |
| --- | --- |
| Fastest on bare WSGI? | **A tie** — one worker and one handler thread each, Granian to m0serve is ~<!-- num:granian-per-m0@2 -->0.98<!-- /num -->x per core and <!-- num:granian-vs-m0-rps@2 -->0.99<!-- /num -->x in requests per second, inside the spread between recordings; the previous recording had Granian a few percent ahead. <!-- observed: notes/the-conclusions-on-linux.md, artifacts in bench/results/linux-2026-09/ -->A first Linux run put m0serve ahead here; a second, on x86-64 hardware, did not reproduce it and agrees with this box, so the answer stands |
| Fastest on bare ASGI? | **In requests per second, yes**: <!-- num:asgi-vs-uvloop@2 -->1.62<!-- /num -->x uvicorn with uvloop (what `pip install uvicorn[standard]` runs) and <!-- num:asgi-vs-uvicorn@2 -->2.31<!-- /num -->x `uvicorn --loop asyncio` at 16 connections, the executor's two threads using <!-- num:asgi-m0-cores@1 -->1.6<!-- /num --> cores where uvicorn has one. **Per core on this box, against uvloop, no**: uvloop is ahead by ~<!-- num:uvloop-per-core-lead@2 -->1.01<!-- /num -->x — <!-- observed: notes/the-conclusions-on-linux.md, artifacts in bench/results/linux-2026-09/ -->but that is macOS-specific: two Linux environments answer at or above parity (1.11x and 1.01x), so uvloop's per-core lead does not survive the platform change, though its size is unsettled. Against `--loop asyncio` the executor leads per core by ~<!-- num:asgi-per-core-vs-uvicorn@2 -->1.40<!-- /num -->x everywhere measured |
...
- the HTTP layer with no Python at all (`apps/hello`) runs at
  **<!-- num:hello-rps-k@1 -->209.5<!-- /num -->k rps/core**, above Granian's end-to-end
  **<!-- num:granian-rps-k@1 -->109.7<!-- /num -->k**
- the same bare WSGI application run inline on that loop, one thread, runs
  at **<!-- num:m0-loop-rps-k@1 -->126.1<!-- /num -->k rps/core**, so **the bridge costs
  <!-- num:bridge-tax@2 -->1.66<!-- /num -->x**
- give the worker one handler thread, Granian's shape, and m0serve serves
  **<!-- num:m0-w1-rps-k@1 -->199.3<!-- /num -->k rps on <!-- num:m0-w1-cores@2 -->1.78<!-- /num --> cores** against Granian's
  **<!-- num:granian-w1-rps-k@1 -->197.4<!-- /num -->k on <!-- num:granian-w1-cores@2 -->1.80<!-- /num -->**: **<!-- num:m0-per-granian@2 -->1.02<!-- /num -->x per
  core**, <!-- num:m0-vs-granian-rps@2 -->1.01<!-- /num -->x in throughput
- zero-config, what `m0serve app.wsgi` runs, serves **<!-- num:m0-zero-config-rps-k@1 -->198.8<!-- /num -->k
  rps** on a pool of eight handler threads

What the split prices is the bridge: the
<!-- num:bridge-tax@2 -->1.66<!-- /num -->x on the inline row is real and
...
<!-- num:w1-rps-gap-pct@1 -->1.0<!-- /num --> % apart in throughput. The
...
Source: [`asgi-wrk-hello-20260925T002524Z.json`](../bench/results/asgi-wrk-hello-20260925T002524Z.json) — 2026-09-25T00:25:24+00:00, commit `9b051fd`.
Environment: Python 3.13.6; Apple M4 (10 cores: 4P+6E); 16 GB; macOS 27.0 (26A428); AC Power; wrk -t2 -c16 -d8s, browser headers; executor loop: uvloop.

| row | rps | cores | rps/core |
|-----|----:|------:|---------:|
| `m0serve` — zero-config executor (its loop is stamped above) | 138,241 | 1.64 | 84,293 |
| `uvicorn --loop asyncio` | 59,745 | 0.99 | 60,349 |
| `uvicorn` with uvloop — what `pip install uvicorn[standard]` runs by default | 85,313 | 1.00 | 85,313 |
...
<!-- num:asgi-m0-cores@2 -->1.64<!-- /num --> cores:
**<!-- num:asgi-vs-uvicorn@2 -->2.31<!-- /num -->x `uvicorn --loop asyncio`**
and <!-- num:asgi-vs-uvloop@2 -->1.62<!-- /num -->x uvicorn with uvloop in
requests per second, <!-- num:asgi-per-core-vs-uvicorn@2 -->1.40<!-- /num -->x
and <!-- num:asgi-per-core-vs-uvloop@2 -->0.99<!-- /num -->x per core. Read
...
edge. Under wrk the ratio is <!-- num:asgi-vs-uvicorn@2 -->2.31<!-- /num -->x at 16 connections (0.72x before the
...
Source: [`asgi-executor-20260925T002602Z.json`](../bench/results/asgi-executor-20260925T002602Z.json) — 2026-09-25T00:26:02+00:00, commit `9b051fd`.
Environment: Python 3.13.6; Apple M4 (10 cores: 4P+6E); 16 GB; macOS 27.0 (26A428); AC Power; seconds=8, threads=8.

| server | rps | fast p50 | fast p99 | errors |
|--------|----:|---------:|---------:|-------:|
| `m0serve` — asyncio executor | 31,826 | 133 µs | 319 µs | 0 |
| `uvicorn` | 24,907 | 170 µs | 374 µs | 0 |
...
to <!-- num:isolation-nopool-slow2-ms@0 -->195<!-- /num --> ms as slow views
...
<!-- num:isolation-ratio@0 -->151<!-- /num -->x change and the largest
...
Source: [`mixed-workload-20260925T005453Z.json`](../bench/results/mixed-workload-20260925T005453Z.json) — 2026-09-25T00:54:53+00:00, commit `9b051fd`.
...
|---|---|---|---|
| `--workers 4` | 0.6 ms (0.6–0.7) | 190.1 ms (189.4–190.5) | 195.1 ms (193.9–195.3) |
| `--threads 4` | 0.7 ms (0.7–0.7) | 193.3 ms (188.6–193.3) | 199.0 ms (197.6–201.1) |
| `--workers 4 +bt=4` | 1.4 ms (1.3–1.4) | 1.3 ms (1.3–1.3) | 1.3 ms (1.3–1.4) |
| `--threads 4 +bt=4` | 0.9 ms (0.9–0.9) | 0.9 ms (0.9–0.9) | 0.9 ms (0.9–0.9) |
| `--workers 1 +bt=4` | 0.5 ms (0.5–0.5) | 0.4 ms (0.4–0.4) | 0.5 ms (0.5–0.5) |
| `granian bt=4` | 0.6 ms (0.5–0.6) | 0.5 ms (0.5–0.5) | 0.5 ms (0.5–0.5) |
