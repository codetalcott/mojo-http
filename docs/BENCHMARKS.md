...
- **Cores are measured, not configured.** Each table's `cores` column is
  ...
  `--workers 1` uses ~<!-- num:granian-w1-cores@2 -->1.78<!-- /num --> cores
...
| question | answer |
| --- | --- |
| Fastest on bare WSGI? | **A tie** — one worker and one handler thread each, Granian to m0serve is ~<!-- num:granian-per-m0@2 -->1.00<!-- /num -->x per core and <!-- num:granian-vs-m0-rps@2 -->1.01<!-- /num -->x in requests per second, inside the spread between recordings; the previous recording had Granian a few percent ahead. <!-- observed: notes/the-conclusions-on-linux.md, artifacts in bench/results/linux-2026-09/ -->A first Linux run put m0serve ahead here; a second, on x86-64 hardware, did not reproduce it and agrees with this box, so the answer stands |
| Fastest on bare ASGI? | **In requests per second, yes**: <!-- num:asgi-vs-uvloop@2 -->1.54<!-- /num -->x uvicorn with uvloop (what `pip install uvicorn[standard]` runs) and <!-- num:asgi-vs-uvicorn@2 -->2.22<!-- /num -->x `uvicorn --loop asyncio` at 16 connections, the executor's two threads using <!-- num:asgi-m0-cores@1 -->1.6<!-- /num --> cores where uvicorn has one. **Per core on this box, against uvloop, no**: uvloop is ahead by ~<!-- num:uvloop-per-core-lead@2 -->1.06<!-- /num -->x — <!-- observed: notes/the-conclusions-on-linux.md, artifacts in bench/results/linux-2026-09/ -->but that is macOS-specific: two Linux environments answer at or above parity (1.11x and 1.01x), so uvloop's per-core lead does not survive the platform change, though its size is unsettled. Against `--loop asyncio` the executor leads per core by ~<!-- num:asgi-per-core-vs-uvicorn@2 -->1.35<!-- /num -->x everywhere measured |
...
- the HTTP layer with no Python at all (`apps/hello`) runs at
  **<!-- num:hello-rps-k@1 -->199.1<!-- /num -->k rps/core**, above Granian's end-to-end
  **<!-- num:granian-rps-k@1 -->108.3<!-- /num -->k**
- the same bare WSGI application run inline on that loop, one thread, runs
  at **<!-- num:m0-loop-rps-k@1 -->122.6<!-- /num -->k rps/core**, so **the bridge costs
  <!-- num:bridge-tax@2 -->1.62<!-- /num -->x**
- give the worker one handler thread, Granian's shape, and m0serve serves
  **<!-- num:m0-w1-rps-k@1 -->191.1<!-- /num -->k rps on <!-- num:m0-w1-cores@2 -->1.76<!-- /num --> cores** against Granian's
  **<!-- num:granian-w1-rps-k@1 -->192.7<!-- /num -->k on <!-- num:granian-w1-cores@2 -->1.78<!-- /num -->**: **<!-- num:m0-per-granian@2 -->1.00<!-- /num -->x per
  core**, <!-- num:m0-vs-granian-rps@2 -->0.99<!-- /num -->x in throughput
- zero-config, what `m0serve app.wsgi` runs, serves **<!-- num:m0-zero-config-rps-k@1 -->185.9<!-- /num -->k
  rps** on a pool of eight handler threads

What the split prices is the bridge: the
<!-- num:bridge-tax@2 -->1.62<!-- /num -->x on the inline row is real and
...
<!-- num:w1-rps-gap-pct@1 -->0.8<!-- /num --> % apart in throughput. The
...
Source: [`asgi-wrk-hello-20260915T153621Z.json`](../bench/results/asgi-wrk-hello-20260915T153621Z.json) — 2026-09-15T15:36:21+00:00, commit `d0608ed`.
Environment: Python 3.13.6; Apple M4 (10 cores: 4P+6E); 16 GB; macOS 26.6.2 (25G83); AC Power; wrk -t2 -c16 -d8s, browser headers; executor loop: uvloop.

| row | rps | cores | rps/core |
|-----|----:|------:|---------:|
| `m0serve` — zero-config executor (its loop is stamped above) | 129,920 | 1.61 | 80,695 |
| `uvicorn --loop asyncio` | 58,609 | 0.98 | 59,805 |
| `uvicorn` with uvloop — what `pip install uvicorn[standard]` runs by default | 84,400 | 0.99 | 85,253 |
...
**<!-- num:asgi-vs-uvicorn@2 -->2.22<!-- /num -->x `uvicorn --loop asyncio`**
and <!-- num:asgi-vs-uvloop@2 -->1.54<!-- /num -->x uvicorn with uvloop in
requests per second, <!-- num:asgi-per-core-vs-uvicorn@2 -->1.35<!-- /num -->x
and <!-- num:asgi-per-core-vs-uvloop@2 -->0.95<!-- /num -->x per core. Read
...
edge. Under wrk the ratio is <!-- num:asgi-vs-uvicorn@2 -->2.22<!-- /num -->x at 16 connections (0.72x before the
...
Source: [`asgi-executor-20260915T153700Z.json`](../bench/results/asgi-executor-20260915T153700Z.json) — 2026-09-15T15:37:00+00:00, commit `d0608ed`.
Environment: Python 3.13.6; Apple M4 (10 cores: 4P+6E); 16 GB; macOS 26.6.2 (25G83); AC Power; seconds=8, threads=8.

| server | rps | fast p50 | fast p99 | errors |
|--------|----:|---------:|---------:|-------:|
| `m0serve` — asyncio executor | 31,086 | 138 µs | 238 µs | 0 |
| `uvicorn` | 24,861 | 169 µs | 379 µs | 0 |
...
fast-route p99 climbs from <!-- num:isolation-nopool-slow0-ms@1 -->0.7<!-- /num --> ms
...
<!-- num:isolation-ratio@0 -->144<!-- /num -->x change and the largest
...
Source: [`mixed-workload-20260915T161637Z.json`](../bench/results/mixed-workload-20260915T161637Z.json) — 2026-09-15T16:16:37+00:00, commit `e81fb04`.
Environment: Python 3.14.7 free-threading build; granian 2.8.2; Apple M4 (10 cores: 4P+6E); 16 GB; macOS 26.6.2 (25G83); AC Power; wrk -c16 -d10s, 3 rounds, medians.

| configuration | slow=0 | slow=1 | slow=2 |
|---|---|---|---|
| `--workers 4` | 0.7 ms (0.6–0.7) | 190.8 ms (189.2–191.3) | 194.9 ms (194.0–195.1) |
| `--threads 4` | 0.7 ms (0.7–0.7) | 193.7 ms (188.9–193.9) | 198.2 ms (196.8–200.3) |
| `--workers 4 +bt=4` | 1.4 ms (1.4–1.5) | 1.4 ms (1.4–1.4) | 1.4 ms (1.3–1.4) |
| `--threads 4 +bt=4` | 0.9 ms (0.8–1.5) | 0.9 ms (0.9–0.9) | 0.9 ms (0.9–0.9) |
| `--workers 1 +bt=4` | 0.5 ms (0.5–0.5) | 0.5 ms (0.5–0.5) | 0.5 ms (0.5–0.5) |
| `granian bt=4` | 0.6 ms (0.6–0.6) | 0.5 ms (0.5–0.5) | 0.5 ms (0.5–0.5) |
