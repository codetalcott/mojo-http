...
| D14 | `ViewService` does not conform to `PoolHandler`; a mounted application is a three-line struct in its mount module (`m0serve_mount`, SPEC N14) holding a `Views` table and its per-thread state | [one-renderer-two-transports](notes/one-renderer-two-transports.md) | standing | the trait boundary lifting, and an app whose state is a per-thread SQLite `Connection` |
