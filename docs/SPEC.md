...
| N11 | Streaming from a Mojo mount: a view on a mounted `Views` table takes a Server-Sent Events hold from a pool thread the way a WSGI pool thread does — the `h` frame on the loop's bus channel — so an application's `LISTEN/NOTIFY` views can move onto the mount; decided by a probe, and if the probe fails a compute path mounts first and the note records why | planned | ROADMAP: Streaming from a Mojo mount |
