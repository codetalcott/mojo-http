...
| N13 | Sessions and CSRF behind a login: a stateless signed cookie `v1.<kid>.<exp>.<subject>.<tag>` — HMAC-SHA256 over the rest, the tag checked in constant time, the expiry against the host clock, a retired key ending every session under it — emitted through `ResponseCookieJar.add_raw` with `HttpOnly; SameSite=Lax; Path=/`; a CSRF token derived from that tag, carried as a hidden field and read from the body alone; a guard that is an early return, answering 303 to a navigation and 401 carrying the login fragment to a swap; and a server that refuses to start (78) without its key or its password | verified | `Smoke test the fragment notes app` (every PR) — every arm on the wire, with the forgeries signed by `scripts/notes_session.py` so the verifier is never checked against itself; the unit form is `test_session.mojo:test_a_cookie_the_issuer_signed_names_its_subject` against the same issuer's vectors, and `poe sabotage-notes-login` (pre-release) reverts the tag check, the expiry, the token, the guard and the cookie attributes one at a time. The arm curl cannot judge is `browser-notes-login` (pre-release): Chromium signs in, writes and deletes, and the delete carries its token in the BODY — htmx 2.0.4 would put it in the query string, which the shell's `htmx-config` meta narrows away. The design is [a-login-on-the-notes-app](notes/a-login-on-the-notes-app.md) |
...
| id | capability | status | evidence |
|---|---|---|---|
| O1 | `open` promises WAL and raises when it cannot deliver it: `PRAGMA journal_mode` answers with the mode actually in force, and a database that stays on a rollback journal — a temp or in-memory database, some network filesystems — is an error at open rather than reader/writer concurrency that silently is not there | verified | `test_file.mojo:test_open_reports_wal_it_could_not_apply` (every PR) — the discarded row is the defect: `open` would promise WAL while delivering a lock that serializes, which surfaces later as inexplicable contention rather than as an error here |
