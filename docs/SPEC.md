...
| M18 | A native build flavor, `poe build-serve-native`: the build host's CPU and OS floor, for a server on a desktop Mac, beside the portable wheel. A local build or a release tarball, never a second wheel, because pip cannot select a microarchitecture | verified | `Smoke test Apple Silicon topology, QoS and the native build flavor` (every PR) — the native binary launches, and its `build.apple_target` names the host's generation where the wheel flavor on the same machine said m1 |
