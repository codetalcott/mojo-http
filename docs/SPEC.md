...
| E16 | Workers sharing one listener share its connections: a burst of keep-alive connections spreads across `--workers N` to within 2:1 on both platforms, at no cost to a single worker | planned | ROADMAP: Workers sharing a listener share its connections |
