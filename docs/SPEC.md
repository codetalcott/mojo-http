...
| N13 | Sessions and CSRF behind a login: an HMAC-SHA256 in `m0-core` gated by published test vectors, a signed session cookie emitted through `ResponseCookieJar.add_raw`, and a CSRF token on the form, on `apps/fragment_notes` with one user | planned | ROADMAP: Sessions and CSRF behind a login |
