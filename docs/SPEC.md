...
| N8 | A history restore is a page: `HX-History-Restore-Request: true` beside `HX-Request: true` answers the whole document, because htmx swaps the answer's body into the page it is rebuilding and a bare fragment there is a page with no head; every answer's `Vary` names all three headers the decision reads | verified | `Smoke test the fragment notes app` (every PR) — `test_fragment.mojo:test_a_history_restore_is_a_page` is the unit form; the request shape is htmx 2.0.4's own `loadHistoryFromServer`, which sets both headers |
