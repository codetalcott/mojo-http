/* Compile-time proof of every struct offset the Mojo vtab spike hardcodes.
 *
 * The spike treats sqlite3_module, sqlite3_vtab, sqlite3_vtab_cursor and
 ...
 * fails the build if any disagrees.
 *
 * Checked with the frontend only — no linker, no sysroot, no runner needed:
 *
 *   for t in x86_64-linux-gnu aarch64-linux-gnu arm64-apple-macos; do
 *     clang -fsyntax-only -target $t experiments/sqlite-vtab/verify_layout.c \
 *       && echo "$t OK"
 *   done
 *
 * Add a triple here rather than trusting that "64-bit is 64-bit": this repo
 ...
 * The spike indexes this as 8-byte words, which requires iVersion to be
 * padded out to 8 before the first callback. M_* names are the spike's.
 */
 ...
 *
 * Note this assertion is stricter than the spike actually needs. What SQLite
...
/* --- sqlite3_vtab_cursor: the spike appends its own fields after it ----
...
/* --- sqlite3_index_info, indexed as words by x_best_index -------------- */
...
/* --- the two inner arrays x_best_index strides over -------------------- */
