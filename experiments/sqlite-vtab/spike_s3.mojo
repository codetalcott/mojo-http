...
# --- S3: does the vtab actually pay for itself? -------------------------------

comptime REPS: Int = 20
comptime TABLE_ROWS: Int = 200_000


def _seed_table(mut db: Connection) raises:
...
def _ids(n: Int) -> List[Int]:
    ...
    for i in range(n):
        out[i] = (i * 7919) % TABLE_ROWS
    return out^


def _placeholders(n: Int) -> String:
    var s = String("?")
    for _ in range(n - 1):
        s += ",?"
    return s^
...
def _json_array(ids: List[Int]) -> String:
    var s = String("[")
...
def bench_in_clause(mut db: Connection, n: Int) raises:
    print("\n--- IN-clause, N =", n, "against", TABLE_ROWS, "rows ---")
    var ids = _ids(n)

    # A. rebuild SQL with N placeholders, prepare, bind N, step
    var best_ph = 0
    for r in range(REPS):
        var t0 = perf_counter_ns()
        var q = db.prepare(
            "SELECT sum(v) FROM t WHERE id IN (" + _placeholders(n) + ")"
        )
        for i in range(n):
            q.bind_int(i + 1, ids[i])
        _ = q.step()
        var s = q.column_int(0)
        var dt = perf_counter_ns() - t0
        if r == 0 or dt < best_ph:
            best_ph = dt
    print("  rebuilt IN (?,...)      ", Float64(best_ph) / 1000.0, "us")

    # B. json_each with a stable statement, JSON built each call
    var q_json = db.prepare(
        "SELECT sum(v) FROM t WHERE id IN (SELECT value FROM json_each(?1))"
    )
    var best_js = 0
    for r in range(REPS):
        var t0 = perf_counter_ns()
        q_json.reset()
        var js = _json_array(ids)
        q_json.bind_text(1, js)
        _ = q_json.step()
        var s = q_json.column_int(0)
        var dt = perf_counter_ns() - t0
        if r == 0 or dt < best_js:
            best_js = dt
    print("  json_each(?)            ", Float64(best_js) / 1000.0, "us")

    # C. the Mojo vtab: stable statement, one pointer bind, zero copies
    var q_vt = db.prepare(
    ...
    for r in range(REPS):
        var t0 = perf_counter_ns()
        q_vt.reset()
        bind_array(q_vt, 1, ids)
        q_vt.bind_int(2, n)
        _ = q_vt.step()
        var s = q_vt.column_int(0)
        var dt = perf_counter_ns() - t0
        if r == 0 or dt < best_vt:
            best_vt = dt
    _ = len(ids)  # keep-alive
    print("  mojo_int64 vtab         ", Float64(best_vt) / 1000.0, "us")
    print(
        "  vtab vs placeholders    ",
        Float64(best_ph) / Float64(best_vt),
        "x",
    )
    print("  vtab vs json_each       ", Float64(best_js) / Float64(best_vt), "x")
...
def bench_ingest(n: Int) raises:
    ...
    # A. the existing idiom: prepared INSERT, bind/step/reset in one transaction
    var best_loop = 0
    for r in range(5):
        var db = open_memory()
    ...
    # B. one INSERT ... SELECT over the vtab: a single step from Mojo
    var best_vt = 0
    for r in range(5):
        ...
        db.begin()
        var ins = db.prepare(
            "INSERT INTO t SELECT value FROM mojo_int64(?1, ?2)"
        )
        bind_array(ins, 1, data)
        ins.bind_int(2, n)
        _ = ins.step()
        _ = ins^
        db.commit()
        var dt = perf_counter_ns() - t0
        _ = len(module)  # keep-alive
        if r == 0 or dt < best_vt:
            best_vt = dt
    _ = len(data)  # keep-alive
    print("  INSERT..SELECT vtab     ", Float64(best_vt) / 1.0e6, "ms")
    print("  speedup                 ", Float64(best_loop) / Float64(best_vt), "x")


def main() raises:
    print("=== S3: does the Mojo vtab pay for itself? (best of", REPS, ") ===")
    ...
    bench_in_clause(db, 10)
    bench_in_clause(db, 100)
    bench_in_clause(db, 1000)
    _ = len(module)  # keep-alive
